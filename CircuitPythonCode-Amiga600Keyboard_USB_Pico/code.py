...
# Debounce/scan timing
ROW_SETTLE_TIME = 0.0005
DEBOUNCE_TIME = 0.008
...
class keyboard:
    def __init__(self):
        ...
        self.__keycodes = {
            kc: getattr(Keycode, kc)
            for kc in dir(Keycode)
            if not kc.startswith("__") and not callable(getattr(Keycode, kc))
        }
        ...
        self.__consumercontrolcodes = {
            c: getattr(ConsumerControlCode, c)
            for c in dir(ConsumerControlCode)
            if not c.startswith("__") and not callable(getattr(ConsumerControlCode, c))
        }
        ...
        self.__additionalconsumercontrolcodes = {
            "CALC": 0x192,
        }
...
def debouncematrix(matrix, lastmatrix, stablematrix, changetime):
    """debouncematrix

    Tracks raw matrix changes and only allows a new stable state once the
    matrix has remained unchanged for DEBOUNCE_TIME seconds.

    returns lastmatrix, stablematrix, changetime
    """
    now = time.monotonic()
    ...
    if matrix != stablematrix and now - changetime >= DEBOUNCE_TIME:
        stablematrix = matrix

    return lastmatrix, stablematrix, changetime
...
def a600keyboard():
    ...
    lastmatrix1 = oldmatrix1
    lastmatrix2 = oldmatrix2

    stablematrix1 = oldmatrix1
    stablematrix2 = oldmatrix2

    changetime1 = time.monotonic()
    changetime2 = time.monotonic()
    ...
    while 1:
        ...
        lastmatrix1, newstablematrix1, changetime1 = debouncematrix(matrix1, lastmatrix1, stablematrix1, changetime1)

        if newstablematrix1 != stablematrix1:
            LAYER = keystatechange(newstablematrix1, stablematrix1, KEYMAP_M1, LAYER)
            stablematrix1 = newstablematrix1

        matrix2 = scanmatrix(rows2, cols2)
        lastmatrix2, newstablematrix2, changetime2 = debouncematrix(matrix2, lastmatrix2, stablematrix2, changetime2)

        if newstablematrix2 != stablematrix2:
            LAYER = keystatechange(newstablematrix2, stablematrix2, KEYMAP_M2, LAYER)
            stablematrix2 = newstablematrix2

        oldmatrix1 = stablematrix1
        oldmatrix2 = stablematrix2
